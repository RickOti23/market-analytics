...
import os
import talib 
from scripts.data_repo import DataRepo
...
class TransformData:
  ...
  def __init__(self, repo:DataRepo):
        ...
        self.stocks_df = repo.stocks_df.copy(deep=True)
        self.macro_df = repo.macro_df.copy(deep=True)
        self.indices_df = repo.indices_df.copy(deep=True)
        # init transformed_df
  ...
  def _get_talib_pattern_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    pattern_indicators_df['Date'] = pd.to_datetime(
            pattern_indicators_df['Date']).dt.tz_localize(None)
  ...
  def _transform_tickers(self):
    ...
    self.stocks_df['Volume'] = self.stocks_df['Volume']*1.0

    # Check which columns are available
    available_columns = ['Open','High','Low','Close','Volume']
    for key in available_columns:
      if key in self.stocks_df.columns:
        self.stocks_df.loc[:,key] = self.stocks_df[key].astype('double')
    ...
    tickers = tqdm(self.stocks_df.Ticker.unique())
    for ticker in tickers:
      tickers.set_description(ticker)
      filter = (self.stocks_df.Ticker == ticker)
      current_ticker_df = self.stocks_df[filter]
      ...
      current_ticker_df['Date']= pd.to_datetime(current_ticker_df['Date']).dt.tz_localize(None)
      df_current_ticker_momentum_indicators['Date']= pd.to_datetime(df_current_ticker_momentum_indicators['Date']).dt.tz_localize(None)
      df_current_ticker_volume_indicators['Date']= pd.to_datetime(df_current_ticker_volume_indicators['Date']).dt.tz_localize(None)
      df_current_ticker_pattern_indicators['Date']= pd.to_datetime(df_current_ticker_pattern_indicators['Date']).dt.tz_localize(None)
     ...
     # merge to one df
      ...
      m3["Date"] = pd.to_datetime(m3["Date"]).dt.tz_localize(None)
  ...
  def _merge_tickers_macro_indexes_df(self):
    ...
    cols = ["Date"]
    for a in cols:
       if a in self.macro_df:
          print(f'Available {a} col')
       else:
          print(f'No {a} in data merging will fail')
    ...
    self.macro_df = self.macro_df.drop(columns="Month", errors="ignore")
    self.indices_df.set_index('Date', inplace=True)
    del self.indices_df["index"]

    #Merging with macro_data
