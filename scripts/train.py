...
class TrainModel:
  ...
  def _define_feature_sets(self):
    ...
    self.GROWTH = [g for g in self.transformed_df.keys() if (g.find('growth_')==0)&(g.find('future')<0)]
    ...
    self.MACRO = ['gdppot_us_yoy', 'gdppot_us_qoq', 'cpi_core_yoy', 'cpi_core_mom', 'FEDFUNDS',
                  'DGS1', 'DGS5', 'DGS10']
    self.CUSTOM_NUMERICAL = ['SMA10', 'SMA20', 'growing_moving_average', 'high_minus_low_relative','volatility', 'ln_volume']
  ...
  def _define_dummies(self):
    self.transformed_df["Month"] = pd.to_datetime(self.transformed_df["Month"], errors="coerce")
    self.transformed_df.loc[:,'Month'] = self.transformed_df.Month.dt.strftime('%B')
  ...
  def prepare_dataframe(self):
    ...
    return
  def train_random_forest(self, max_depth=15, n_estimators=150):
    # https://scikit-learn.org/stable/modules/ensemble.html#random-forests-and-other-randomized-tree-ensembles
    print('Training the best model (RandomForest (max_depth=15, n_estimators=150))')
  ...
  def make_inference(self, pred_name:str):
    ...
    y_pred_all_class1 = [k[1] for k in y_pred_all] #list of predictions 
