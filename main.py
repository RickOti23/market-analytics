...
def main():
  ...
  if TRANSFORM_DATA:
    transformed.fetch()
    transformed.persist(data_dir='data')
  else:
    transformed.load(data_dir='data')
  ...
  if TRAIN_MODEL:
    ...
    trained.persist(data_dir='data') # save the model to disk
  else:
    trained.prepare_dataframe() # prepare dataframes (incl. for inference)
    trained.load(data_dir='data')
  ...
  prediction_name='RandomPredicts'
